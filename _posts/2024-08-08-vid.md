...
For some use cases *totality of dispersal* is not needed, and termination of retrieval is weakened so that even if one honest party terminated the dispersal, all will complete the retrieval. We will call this variation, *provable but un-total AVID*. (See [NNT22](https://eprint.iacr.org/2021/1544) for example.) See the end of this post for why it is crucial for efficient BFT protocols.
