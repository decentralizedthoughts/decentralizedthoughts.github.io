...
We have now arrived at a single-shot protocol that captures the core ideas of Simplex. For the multi-shot version, see the [Simplex paper](https://eprint.iacr.org/2023/463), [Simplex blog post](https://simplex.blog/), or [Victor Shoup's paper](https://eprint.iacr.org/2023/1916).)

The [original Simplex protocol](https://simplex.blog/) has other differences from Tendermint. For example, the (View-change, k) message is called (Vote, k, $\bot$) for notational convenience, and a party may vote in view $k$ even after sending (Vote, k, $\bot$). This post focuses on a minimal set of changes (from Tendermint) to illustrate the essence of Simplex, and omits these inconsequential differences. 
In fact, the version of Simplex presented in this post is arguably more natural. It is more appropriate to call the view-change message (Finalize, k, $\bot$), given that it is mutually exclusive with (Finalize, k, x). A separate (Vote, k, $\bot$) message (that is mutually exclusive with (Vote, k, x)) can then be introduced to improve both optimistic and worst-case latency (see [this post](https://decentralizedthoughts.github.io/2025-07-29-2-round-3-round-simplex/) and [this post](https://decentralizedthoughts.github.io/2025-09-24-Kuplex/)). The original Simplex also describes its chained multi-shot consensus in an unconventional way. [Shoup's paper](https://eprint.iacr.org/2023/1916) gives an alternative description. 
...
**Acknowledgment.** The post is written during the author's visit to a16z Crypto Research. The author thanks Giulia Scaffino, Kartik Nayak, Joachim Neu, and Ittai Abraham for their suggestions on improving the post. 
