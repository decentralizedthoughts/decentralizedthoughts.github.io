...
* **Safety:** For any two valid commit certificates $C2_{v}(B_{k})$ and $C2_{v'}(B_{k'}')$, either $B_k=B_{k'}'$ or $B_{k}$ extends $B_{k'}'$ or $B_{k'}'$ extends $B_k$. In other words: $B_k$ and $B_{k'}'$ do not equivocate.
