...
* **Safety:** For any two valid commit certificates $D_{v}(B_{k})$ and $D_{v'}(B'_{k'})$, either $B_k=B'_{k'}$ or $B_{k}$ extends $B'_{k'}$ or $B'_{k'}$ extends $B_k$. In other words: $B_k$ and $B'_{k'}$ do not equivocate.
