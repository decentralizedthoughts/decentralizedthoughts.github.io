...
When a pending message $e=(m,p)$ is delivered, then party $p$ receives message $m$ in its incoming-messages queue.
...
Are all initial configurations either 1-committed or 0-committed? The following lemma shows that this is not the case: every protocol that can tolerate even one crash failure must have some initial configuration that is **uncommitted**.
